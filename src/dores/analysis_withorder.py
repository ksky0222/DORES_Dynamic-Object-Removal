...
if __name__ == "__main__":
    ...
    data_dir = source_dir + "src/DORES/map_data/"
    gt = data_dir + "gt/" +sys.argv[1] +"_voxel_0_2.pcd"
    est = data_dir + sys.argv[1] + "/" + sys.argv[1] + "_dynamic_removed_withorder.pcd"

    assert os.path.isfile(gt), "GT path does not exist"
    assert os.path.isfile(est), "Est path does not exist"

    gt_data = load_pcd(gt)
    target_data = load_pcd(est)
